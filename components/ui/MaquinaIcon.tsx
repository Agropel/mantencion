...
// export default function MaquinaIcon({maquina}: MaquinaIcon ) {
// const params = useParams<{maquina: string}>()
//   return (
//     <div
//         className={`${maquina.id === params.maquina ? 'bg-green-600' : ''} flex justify-center items-center gap-4 w-full border-t border-green-300 p-3 last-of-type:border-b`}
//     >
//             <Link
//                 className="text-xl font-bold"
//                 href={`/order/${maquina.id}`}
//             >{maquina.id}</Link>
//         </div>
//   )
// }
        ...
        <div className="p-4">
            ...
            {filteredMaquinas.length > 0 ? (
                filteredMaquinas.map(maquina => (
                    ...
                    >
                        <Link className="text-xs font-bold" href={`/order/${maquina.id}`}>
                            {maquina.name}
                        </Link>
                    </div>
                ))
